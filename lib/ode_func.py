...
class ODEFunc_w_Poisson(ODEFunc):
    def __init__(self,
		...
		"""
        super(ODEFunc_w_Poisson, self).__init__(input_dim, latent_dim,
                                                ode_func_net, device)

        self.latent_ode = ODEFunc(ode_func_net=ode_func_net, device=device)
    ...
    def extract_poisson_rate(self, augmented, final_result=True):
        ...
        if len(augmented.size()) == 3:
            ...
            log_lambdas = self.lambda_net(
                y_latent_lam[:, :, :, -latent_lam_dim:])
            y = y_latent_lam[:, :, :, :-latent_lam_dim]
