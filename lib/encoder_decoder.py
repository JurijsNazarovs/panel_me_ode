...
'''
Classes: GRU_unit, Encoder_z0_RNN, and Encoder_z0_ODE_RNN
are borrowed from https://github.com/YuliaRubanova/latent_ode
'''
...
class GRU_unit(nn.Module):
    # Unit is used in Encoder_z0_ODE_RNN below
    def __init__(self,
                 latent_dim,
                 input_dim,
                 update_gate=None,
    ...
    def forward(self, y_mean, y_std, x, masked_update=True):
        ...
        if masked_update:
            ...
            # update only the hidden states for hidden state only if
            # at least one feature is present for the current time point
...
class Encoder_z0_RNN(nn.Module):
    # RNN encoder for initial point of ODE z0
    def __init__(self,
                 latent_dim,
                 input_dim,
                 lstm_output_size=20,
...
class Encoder_z0_ODE_RNN(nn.Module):
    # Derive z0 by running ode backwards trhough observing data`.
    # For every y_i we have two versions: encoded from data and
    # derived from ODE by running it backwards from t_i+1 to t_i
    # Compute a weighted sum of y_i from data and y_i from ode.
    # Use weighted y_i as an initial value for ODE runing from t_i to t_i-1
    ...
    def __init__(self,
        ...
        self.transform_z0 = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Linear(100, self.z0_dim * 2),  #because of mu and sigma
        )
    ...
    def forward(self, data, time_steps, run_backwards=True, save_info=False):
        ...
        if torch.isnan(data).any():
            import pdb
            pdb.set_trace()
            print("Debug data in Encoder_z0_ODE_RNN")
            raise Exception("Data in forward encoder is Nan!")
        assert (not torch.isnan(time_steps).any())
        ...
        mean_z0, rho_z0 = utils.split_last_dim(
            self.transform_z0(torch.cat((means_z0, std_z0), -1)))
        if save_info:
            self.extra_info = extra_info

        return mean_z0, rho_z0
    ...
    def run_odernn(self,
        ...
        for i in time_points_iter:
            if (prev_t - t_i) < minimum_step:
                ...
                time_points = utils.linspace_vector(
                    prev_t, t_i, n_intermediate_tp).to(device)
...
class Decoder(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        # decode data from latent space where we are solving an ODE
        # back to the data space

        decoder = nn.Sequential(nn.Linear(input_dim, output_dim), )
...
# ===== 2D Encoder/Decoder - cnn+fc, used to encode image in 1d vector ===== #
class Encoder2d(nn.Module):
    def __init__(self, input_dim, output_dim, conv_dim=10, ks=3):
        super(Encoder2d, self).__init__()
        # Encodoe 2d image in 1d data for RNN

        encoder = nn.Sequential(
            nn.Conv2d(input_dim, 12, ks, stride=1, padding=1),  # 4, 28, 28
            nn.ReLU(),  #
            nn.Conv2d(12, 24, ks, stride=2, padding=1),  # 16, 14, 14
            nn.ReLU(),
            nn.Conv2d(24, output_dim, ks, stride=2,
                      padding=1),  # output_dim, 7, 7
            #nn.Sigmoid(),
            nn.Flatten(2),  #output_dim, 49
            nn.Linear(49, 1),  #output_dim, 1
    ...
    def forward(self, data):
        # Encoder every time step independently. Thus, to make it faster
        # we reshape time in batch and then back
        n, t = data.shape[:2]
...
class Decoder2d(nn.Module):
    def __init__(self,
        ...
        # Decode from 1d to 2d
        self.extend_to_2d = nn.Linear(input_dim, 49 * input_dim)
        ...
        decoder = nn.Sequential(
            ...
            nn.ConvTranspose2d(24,
                               ...
                               stride=2,
                               padding=1,
                               output_padding=1),  #4, 28, 28
            #nn.ReLU(),
            nn.ConvTranspose2d(12, output_dim, ks, stride=1,
                               padding=1),  #output_dim, 28, 28
            nn.Sigmoid(),
        )
    ...
    def forward(self, data):
        ...
        data = self.extend_to_2d(data)  #bs, inp_dim*7*7
        data = data.reshape(data.shape[:-1] + (-1, 7, 7))  #bs, inp_dim, 7, 7
